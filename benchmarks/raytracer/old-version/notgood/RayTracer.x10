...
public class RayTracer {
	...
	public void render(final Interval interval)
	{
		...
		finish ateach (point(:rank==1)[pl] : U) {
			...
			Ray r = new Ray(view.from, voidVec);
                        ...
                        // Bad place access in the following loop, after many iterations
	...
	private boolean intersect(Ray r, double maxt, Isect inter) {
		...
		inter.t = 1e9;
                ...
                final int[:rank==1] intRecv = new int[dRecv];
                finish async (place.FIRST_PLACE) {
                  final int sstemp = scene.getObjects();
                  async (h) { intRecv[0] = sstemp; }
                }
                final int ii = intRecv[0];

		for (point(:rank==1)[i] : [0:ii-1]) {
			// uses global temporary Prim (tp) as temp.object for speedup
                        // Bad place access in the next line:
                        // final Sphere sph = prim[i];
                        final place h2 = here;
                        final dist(:rank==1) dRecv2 = (dist(:rank==1)) ([0:0]->here);
                        final Sphere[:rank==1] SphereRecv2 = new Sphere[dRecv2];
                        finish async (prim.distribution[i]) {
                          final Sphere sstemp2 = prim[i];
                          async (h2) { SphereRecv2[0] = sstemp2; }
                        }
                        final Sphere sph = SphereRecv2[0];

			Isect tp = sph.intersect(r);
