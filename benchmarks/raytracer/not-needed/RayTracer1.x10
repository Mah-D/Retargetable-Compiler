...
public class RayTracer {
	...
	public void setScene(Scene scene)
	{
		...
		dist(:rank==1) d1 = [0:nLights]->fp;
		dist(:rank==1) d2 = [0:nObjects]->fp;
		//region(:rank==1) r1 = [0:nLights];
	    //region(:rank==1) r2 = [0:nObjects];
		lights = new Light[d1];
		prim = new Sphere[d2];
		/**********************************************************/
	...
	private boolean intersect(Ray r, double maxt, Isect inter) {
		int nhits = 0;
		inter.t = 1e9;
		// Execute on first place
		for (point(:rank==1)[i] : [0:63]) {
	...
	private Vec shade(int level, double weight, Vec P, Vec N, Vec I, Isect hit, Ray tRay) {
		...
		// Execute on first place
		final place pHere = here;
		final dist(:rank==1) dHere = [0:0]->here;
		final Vec[:rank==1] lightPos = (Vec[:rank==1]) (new Vec[dHere]);
		final double[:rank==1] lightBrightness = (double[:rank==1]) (new double[dHere]);
		final int[:rank==1] lightIdx = (int[:rank==1])(new int[dHere]);

		finish async(place.FIRST_PLACE)
		{
			final int lightCount = scene.getLights()-1;
			async(pHere)
			{
				lightIdx[0] = lightCount;
			}			
		}
	
		final int lightIdx1 = lightIdx[0];	
		/********************************************************/

		for (point(:rank==1)[l] : [0:lightIdx1]) {		
			
			/********************************************************/
			finish async(place.FIRST_PLACE)
	        {
				final Vec pos = lights[l].pos;
				final double x = lights[l].pos.x;
				final double y = lights[l].pos.y;
				final double z = lights[l].pos.z;
				final boolean isNull = lights[l].pos.isNull;
				final double brightness = lights[l].brightness;
	            async(pHere)
	            {
					lightPos[0] = pos;
		            //lightPos[0] = new Vec(x,y,z,isNull);
					lightBrightness[0] = brightness;
	            }
	        }

			final Vec lightPos1 = lightPos[0];
			final double lightBrightness1 = lightBrightness[0];
			
			// System.out.println(here + " " + lightPos1);
			
			/********************************************************/			
  
            Vec L = Vec.sub(lightPos1, P);
			if (Vec.dot(N, L) >= 0.0) {
				L = L.normalized();
				double t = L.length();
				...
				if (Shadow(tRay, t, hit) > 0) {
					double diff = Vec.dot(N, L) * surf.kd * lightBrightness1;
		...
		if (surf.kt * weight > 1e-3) {
			if (hit.enter > 0)
				tRay.d = (Vec) (TransDir(new Surface(0,0,0,new Vec(),true), surf, I, N));
			else
				tRay.d = (Vec) (TransDir(surf, new Surface(0,0,0,new Vec(),true), I, N));
