...
class Stream{
	...
	public void run(){
		...
		//final region(:rank==1&&zeroBased&&rect)  RLocal=[0:LocalSize-1];
			...
			//final region(:rank==1) [:rank==1] placeDist = (region(:rank==1) [:rank==1]) new region(:rank==1)[dUnique];
			final region(:rank==1)[:rank==1] placeDist = (region(:rank==1)[:rank==1]) new region(:rank==1)[dUnique](point(:rank==1) p) {return ((tempSd.D)|here).region;};
			finish{
					...
					finish ateach(point(:rank==1) p6: dist.UNIQUE) {
						...
						for (point(:rank==1) p7 : (tempSd.D)|here) {
							...
							tempSd.c[p7] = 2.5*(here.id * LocalSize+p7[0]);
                                                ...
                                                */
						final region(:rank==1) tempReg = (region(:rank==1))placeDist[p6];
						...
						for (point(:rank==1) p4: tempReg) tempSd.a[p4]=tempSd.b[p4]+alpha*tempSd.c[p4];
					}
				...
				finish ateach(point(:rank==1) p2: dist.UNIQUE){
					final region(:rank==1) tempReg = (region(:rank==1))placeDist[p2];
					for (point(:rank==1) p3: tempReg) // verification
