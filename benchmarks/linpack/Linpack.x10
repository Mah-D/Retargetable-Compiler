...
public class Linpack {
    ...
    private void write(final double[:rank==2] a, 
		     final point(:rank==2) p,
		     final double val) {
	//async (a.distribution.get(p)) atomic a[p] = val;
    ...
    */
			...
			final double[:rank==2] b)
    {
        ...
        */
        ...
        finish ateach (point(:rank==2) p : b) { 
            b[p] = 0.0;
        }
        // finish ateach (point(:rank==2) p: a.distribution | [0:n-1,0:n-1]) {
        //     plusWrite(b, 0, j, a[p);
        // }
    ...
    */
	...
	if (job == 0) {
		...
		for (point(:rank==1) [kb] : temprnm1Dist) {
		    final int k = n - (kb+1);
		    final int kp1 = k + 1;
		    b[0,k] = b[0,k] + ddot(n-(kp1), a, k, kp1, 1, b, 0, kp1, 1); //FIXME: subarrays not yet supported
		    final int l = ipvt[k];
		    if (l != k) {
			double t = b[0,l];
			b[0,l] = b[0,k];
    ...
    */
    private void daxpy(final int n, 
		     final double da, 
