...
public class md {
    ...
    public void initialise(int rank0, int nprocess0) {
	rnk = rank0;
	nprocess = nprocess0;
	...
	/* Particle Generation */
	int ijk = 0;
	for (point(:rank==1) [lg] : [0 : 1]) {
	    for (point(:rank==1) [i] : [0 : mm - 1]) {
		for (point(:rank==1) [j] : [0 : mm - 1]) {
		    for (point(:rank==1) [k] : [0 : mm - 1]) {
    ...
    public void runiters2() {
	allreduce();
    }

    public void runiters3(int move) {
	...
	for (point(:rank==1) [j]: P) {
		...
		t.one[k].xforce += dataD[0];
		t.one[k].yforce += dataD[1];
		t.one[k].zforce += dataD[2];
	    }
