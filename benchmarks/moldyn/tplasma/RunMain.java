...
class Dist {
	...
	public MR ()
		...
		//finish 			{
			//for(point()[RX10_TEMP4] : RX10_TEMP2)
...
public static void MR_map ( final MR X10_TEMP0)		{
		...
		//finish 			{
			...
			//for(point()RX10_TEMP0 : RX10_TEMP3)
...
public static void MR_reduce ( final MR X10_TEMP0)		{
		...
		//for(point()RX10_TEMP7 : RX10_TEMP10)
...
throw new RuntimeException(RX10_TEMP36);
		...
		//for(point()RX10_TEMP40 : RX10_TEMP43)
...
public static int[] allocateLocalIntArray(final int count) {
		...
		//final int localReg = [0 : sizeOfChunk];
		final int localReg = sizeOfChunk+1;
		final int allocPlace = 0;
		final int localDist = localReg;
	...
	public static double[] allocateLocalDoubleArray(final int count) {
	 ...
	 public static intStub [] createIntArray(final Dist distArray) {
		...
		final intStub [] temp = new intStub[R];
  	...
  	public static doubleStub [] createDoubleArray(final Dist distArray) {
		...
		final doubleStub [] temp = new doubleStub[R];
	...
	public static int[] createDist(final int reg, final int pl) {
		final int [] placeArray = new int[reg];
		//((point(:rank==1) p)
	...
	public static int[] extendDist(final Dist d1, final int x, final int y, final int pl) {
		...
		int tail = x;
		boolean bool = (tail<=y);
		while(bool) // check the math 0..replace with do-while
		{
			temp[tail] = (pl);
			tail = tail + one;
			bool = (tail <= y);
	...
	public static int [] getRunningSum(final Dist d) {
		...
		//final int runReg = [0 : count];
		final int runReg = count;
		final int runDist = runReg;
		...
		final int [] runSum = new int [runReg];
		//(point(:rank==1) p) 
	...
	public static intStub createintStub(final int remotePlace, final int count) {
		...
		//final int retReg = [0 : zero];
		final int retReg = 1;
		final int retDist = retReg;
