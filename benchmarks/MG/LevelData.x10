...
public value LevelData {	
	...
	private final int m_numPlaces;
		...
		// assert (a_idx>=0 && a_idx<m_numPlaces);
		return (region(:rank==3))get_m_regions(a_idx);  // return (region(:rank==3)) m_regions[a_idx]
	}
	public region(:rank==3) getINNERRegion(final int a_idx){
		// assert (a_idx>=0 && a_idx<m_numPlaces);
	...
	private void print(){
		finish ateach(point(:rank==1) [i]:m_places){
			...
			for (point(:rank==3) p:m_REGIONs[i]) {res = res + Math.abs(temp[p]);}
	...
	public double norm2(){
		...
		finish ateach(point(:rank==1) [i]: m_places){
			...
			for (point(:rank==3) p: get_m_regions(i)) {   // m_regions[i]) {
                            ...
                            finish async (temp.distribution.get(p)) {
...
/*
                ...
                finish async (m_regions.distribution.get(i)) {
...
*/
          		...
          		for (point(:rank==3) p: mri) { 
                            ...
                            async( u.distribution.get(p) ) { u[p]=tp; }
                        }
           	}
           	if (EXCHANGE_After) finish exchange();
        }
	...
	public LevelData(final int a_problemDomainSize, final boolean a_isParallel) {
		...
		for (point(:rank==3) [x,y,z]: temp_m_placeGrid){
		    ...
		    // m_regions[i]=[m_blockSize0*x+m_low0:   m_blockSize0*(x+1)+m_low0-1, 
		    //               m_blockSize1*y+m_low1:   m_blockSize1*(y+1)+m_low1-1,
		    //               m_blockSize2*z+m_low2:   m_blockSize2*(z+1)+m_low2-1];
		    // m_REGIONs[i]=[m_blockSize0*x+m_low0-1: m_blockSize0*(x+1)+m_low0, 
		    //               m_blockSize1*y+m_low1-1: m_blockSize1*(y+1)+m_low1,
		    //               m_blockSize2*z+m_low2-1: m_blockSize2*(z+1)+m_low2];
		...
		final Wrapper [:rank==1] temp_m_u = getWrapperArray(ALLPLACES);
                ...
                for (point(:rank==1) [k] : [0:N_PLACES-1]) {
                        ...
                        finish async(temp_m_u.distribution.get(k)) {
...
*/
      ...
      private double [:rank==3] getDoubleArray(final dist(:rank==3) d) {
              return new double [d] (point(:rank==3) pp) { return 0; };
      }
       ...
       private region(:rank==3) get_m_boundaries(final int i, final int j) {
          ...
          final region(:rank==3)[:rank==1] aRecv =
             (region(:rank==3)[:rank==1]) new region(:rank==3)[dRecv];
          finish async (m_boundaries.distribution.get(i,j)) {
       ...
       private Wrapper get_m_u(final int i) {
          ...
          finish async (m_u.distribution.get(i)) {
