...
public class MGOP {
	...
	private final LevelData [:rank==1] m_r;
	private final LevelData [:rank==1] m_z;
	...
	public MGOP(final int a_size){
		...
		/* building distributed arrays at each refinement level */ 
                final dist(:rank==1) d = [0:m_levels] -> here;
		m_r=new LevelData [d];
                m_z=new LevelData [d];
		j=m_size;
		boolean isParallel = false;
                // for (i=m_levels-1;i>-1;i--){
                i=m_levels-1;
                while (i>-1) {
		        // The level number starts with m_levels-1 and 
                        // ends with 0. At level 0, the size is 2.
	...
	private void ApplyOpQ(final LevelData a_res, final LevelData a_arg){
		...
		finish ateach(point(:rank==1) [i]:a_res.getPlaces()){ /*at each place.*/
			...
			for(point(:rank==3) p:R){//flop: 35 addition+8 multiplication; integer: 36 addition+1 mult
				...
				for (point(:rank==3) o: Util.UNIT_CUBE){
					d=0;
