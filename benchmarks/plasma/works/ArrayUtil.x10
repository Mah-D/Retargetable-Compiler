...
public class ArrayUtil {
	...
	finish ateach (point(:rank==2) [i,j] : dResult) {
	    result[i,j] = new Complex(src[i,2*j], src[i,2*j+1]);
	}
	...
	finish ateach (point(:rank==2) pt[i,j] : dResult) {
	    if (j % 2 == 0) {
		result[pt] = src[i,j/2].getReal(); 
	    } else {
		result[pt] = src[i,j/2].getImag();
	    }
	}
...
//     }
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
		...
		final region(:rank==1) rRSBuffer = [0:place.MAX_PLACES - 1];
		...
		foreach (point(:rank==1) [i] : rSBuffer) {
		    ateach (point(:rank==2) pt : rSBuffer[i]->place.places(i)) {
			result[pt] = sBuffer[pt];
		    }
		}
	    }
