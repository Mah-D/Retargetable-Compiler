public class LinearESOpenPIC2D {
     ...
     */
    private static final int NUMBER_OF_XBITS = 2; // 3
    private static final int NUMBER_OF_YBITS = 3; // 4
     ...
     */
    private static final int PARTICLE_XDENSITY = 4; //  8;
    private static final int PARTICLE_YDENSITY = 8; // 16;
	...
	for (point(:rank==1) [i] : [0 : TIME_STEPS - 1]) {
		...
		moveTime += (System.currentTimeMillis() - time);
	    }
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
	    ...
	    final int koff = particlesPerPlace * pl;
		...
		if (k > 0) {
		    yt += 1.0 / (bny * linearDensityFn(yt, 0.0, 0.0, 0.0));
		}
		...
		double xn = j + x0 + 1;
		if (j > 0) {
		    xt += 1.0 / (bnx * linearDensityFn(xt, 0.0, 0.0, 0.0));
		}
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
	    ...
	    final double anpl = pl * nParticlesAve;
	    final double anpr = (pl + 1) * nParticlesAve;
	...
	finish ateach (point(:rank==2) pt : dIonBG) {
	    chargeDensity[pt] += ionBackground[pt];
	}

	addGuards(chargeDensity, chargeDensityGuards);
	time2 = System.currentTimeMillis();
	depositTime += (time2 - time1);
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
	    ...
	    for (point(:rank==1) pt : particles | here) {
		...
		localCharge[y + 1,x + 1] += charge * dx * dy;
		localCharge[y + 1,x]     += charge * (1.0 - dx) * dy;
		localCharge[y,x + 1]     += charge * dx * (1.0 - dy);
		localCharge[y,x]         += charge * (1.0 - dx) * (1.0 - dy);
	    }

	    // Split the local data to field charge array and guard array
	    finish {
		foreach (point(:rank==2) pt : fieldCharge | here) {
		    fieldCharge[pt] = localCharge[pt];
		}
		foreach (point(:rank==2) pt : guards | here) {
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
	    final place pDst = place.places(pl / 2);
	...
	for (point(:rank==1) [pl] : dist.UNIQUE) 
		...
		final place pDst1 = place.places(2 * pl);
		final place pDst2 = place.places(2 * pl + 1);
	...
	do {
		...
		finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
		    ...
		    if (holesLeft <= 0) {
			nPart[pl] -= holesLeft; // adds some particles
			holesTotal = inLt[pl];  
		    } 
	...
	for (point(:rank==1) pt[pl] : dist.UNIQUE) {
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
	    ...
	    foreach (point(:rank==1) pt : particles | here) {
	...
	final double[:rank==2] result = new double[dResult];
	finish {
	    foreach (point(:rank==2) pt : data | here) {
		result[pt] = data[pt]; 
	    }
	    foreach (point(:rank==2) pt : guards | here) {
		result[pt] = guards[pt];
...
// 	}
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
	    ...
	    int total = 0;
	    for (point(:rank==1) [i] : particleCount | here) {
	...
	do {
		...
		if (lDeficit < 0.0) {
		    ...
		    do {
			oldTotal = total;
			total += sCountBuffer[pl,i];
			i++;
		    } while (total < lDeficit && i < gsize);
		    double offset = i - 1;
		    if (total > oldTotal) {
			offset += (((double) (lDeficit - oldTotal)) / 
				   ((double) (total - oldTotal)));
		    }
		    border[pl] = 
			runningGTotals[pl] - sendGrids[pl] + offset;
		...
		if (!here.isFirst()) {
		    ...
		    runningGTotals[pl] -= sendGrids[pl];
	...
	do {
		...
		if (!here.isLast()) {
		    ...
		    final int gsize = recvGrids[pl];
		    runningPTotals[pl] += count;
		    runningGTotals[pl] += gsize;
		    sendCount[pl] = count;
		    sendGrids[pl] = gsize;
...
// 	    System.out.println(here + " edge: " + edge);
	...
	for (point(:rank==1) [i] : [0 : place.MAX_PLACES - 1]) {
	    ...
	    for (point(:rank==1) [j] : [i + 1 : place.MAX_PLACES - 1]) {
		if (lowerbounds[j] <= lb) {
		    lowerbounds[j] = lb + 1;
		}
	    }
	...
	dist(:rank==2) dResult = 
	    (dist(:rank==2)) (region.factory.emptyRegion(2)->here);
	for (point(:rank==1) [pl] : [0 : place.MAX_PLACES - 1]) {
	    final place p = place.places(pl);
// 	    System.out.println(p+"->"+lowerbounds[pl]+" : "+upperbounds[pl]);
	...
	while (strides < place.MAX_PLACES) {
	    ...
	    finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
		final int dir = (pl / stride) % 2;
		final int val = tmp1[pl];
		final place pDst = 
		    place.places(pl + ((dir == 0) ? 1 : - 1) * stride);
