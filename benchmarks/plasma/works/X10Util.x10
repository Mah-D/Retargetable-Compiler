...
//     }
	...
	final dist(:rank==1) d1 = 
	    (dist(:rank==1)) dist.factory.block(getRank2D(r,0));
     ...
     *  first dimension. 
     */
    public static boolean isDistBlockStar(dist(:rank==2) d) {
	if (ENABLE_CHECKS) {
	    final region(:rank==1) r1 = getRank2D(d.region, 0);
	    final region(:rank==1) r2 = getRank2D(d.region, 1);
		...
		final place p = place.places(pl);
		final region(:rank==2) rLocal = 
		    (region(:rank==2)) (d.distribution | p).region;
     ...
     *  second dimension. 
     */
    public static boolean isDistStarBlock(dist(:rank==2) d) {
	if (ENABLE_CHECKS) {
	    final region(:rank==1) r1 = getRank2D(d.region, 0);
	    final region(:rank==1) r2 = getRank2D(d.region, 1);
	    final dist(:rank==1) dRef = 
     ...
     */
    public static region(:rank==2) transposeRegion(region(:rank==2) r) {
     ...
     *  region.
     */
    public static dist(:rank==2) transposeDist(dist(:rank==2) d) {
	dist(:rank==2) dResult = 
	    (dist(:rank==2)) (region.factory.emptyRegion(2)->here);
	for (point(:rank==1) [pl] : [0 : place.MAX_PLACES - 1]) {
	    final place p = place.places(pl);
     ...
     */
    public static dist(:rank==2) transposeBlockStar(dist(:rank==2) d) {
	final region(:rank==1) r1 = getRank2D(d.region, 0);
	final region(:rank==1) r2 = getRank2D(d.region, 1);
	
	return extendDistRank2D((dist(:rank==1)) dist.factory.block(r2), r1);
    }
