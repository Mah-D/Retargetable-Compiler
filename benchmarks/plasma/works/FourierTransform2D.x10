...
public class FourierTransform2D {
	...
	invN = 1.0 / (2 * length1 * length2);

	tableBitReverse = initBitReverse();
	tableSineCosine = initSineCosine();	
    }
     ...
     *  
     */
    public int value[:rank==1] initBitReverse() {
     ...
     *
     */
    public Complex value[:rank==1] initSineCosine() {
	...
	finish ateach (point(:rank==1) [pl] : dist.UNIQUE) {
		...
		f[i,nxhh] = (f[i,nxhh].getConjg()).multS(2);
		final Complex t2 = f[i,0];
		final double t2Real = t2.getReal();
		final double t2Imag = t2.getImag();
		f[i,0] = new Complex(t2Real + t2Imag, t2Real - t2Imag);
	    }
