public value PoissonSolver {
	...
	final FourierTransform2D fft0 = 
            new FourierTransform2D(yBits + 1, xBits + 1);
	fft = fft0;
     ...
     */
    public double[:rank==3] initTables(final FourierTransform2D fft,
                                       final int x, 
                                       final int y,
                                       final double affp) {
	...
	finish ateach (point(:rank==1) pl: dist.UNIQUE) {
		...
		foreach (point(:rank==2) p: rLocal) {
		    ...
		    //System.out.println(val + " " + p[0] + " " + p[1] + " " + r);
	...
	finish ateach (point(:rank==1) pl: dist.UNIQUE) {
		...
		foreach (point(:rank==2) p: rLocal) {
			...
			final double val = 
                            radialElectricField(r,invRadius,affp);
	...
	//System.out.println(q.distribution);
	...
	for (point(:rank==1) pl: dist.UNIQUE) 
		...
		//System.out.println(q.distribution | here);
     ...
     */
    public double potentialField(final double r, 
                                 final double ari, 
                                 final double affp) {
     ...
     */
    public double particleShape(final double r, 
                                final double ari,
                                final double  affp) {
     ...
     */
    public double radialElectricField(final double r, 
                                      final double ari, 
                                      final double affp) {
	final double anorm = affp * PI4I;	
