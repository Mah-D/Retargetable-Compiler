...
public class IDEATest {
    ...
    public byte [:rank==1] plain1;       // Buffer for plaintext data.
    public byte [:rank==1] crypt1;       // Buffer for encrypted data.
    public byte [:rank==1] plain2;       // Buffer for decrypted data.

    public int value[:rank==1] userkey;  // Key for encryption/decryption.
    public int value[:rank==1] ZR;       // Encryption subkey (userkey derived).
    public int value[:rank==1] DKR;      // Decryption subkey (userkey derived).
	...
	plain1 = new byte[D] (point(:rank==1) p) { return (byte) p[0]; };
	crypt1 = new byte[D];
	plain2 = new byte[D];
    ...
    public void validate() {
        final byte [:rank==1] p1 = plain1;       // Buffer for plaintext data.
        final byte [:rank==1] c1 = crypt1;       // Buffer for encrypted data.
        final byte [:rank==1] p2 = plain2;       // Buffer for decrypted data.

	finish ateach (point(:rank==1) pt : plain1.distribution) {
	    if (p1[pt] != p2[pt]) {
	...
	//JGFInstrumentor.startTimer("Section2:Crypt:Kernel");
        ...
        final int value[:rank==1] fzr = ZR;      // Encryption subkey (userkey derived).
        final int value[:rank==1] fdkr = DKR;    // Decryption subkey (userkey derived).

	finish ateach (point(:rank==1) pt : dist.UNIQUE) {
	    cipher_idea(p1, c1, fzr);     // Encrypt plain1.
	    cipher_idea(c1, p2, fdkr);    // Decrypt.
	}
	...
	final Random rndnum = new Random(136506717L);  // Create random number generator.
	...
	final int value[:rank==1] tempuserkey = (int value[:rank==1]) 
	          new int value[rUserKey] (point(:rank==1) pt) { return tUserKey[pt]; };  // User key has 8 16-bit shorts.
        userkey = tempuserkey;
	...
	int j = 0;                       // Utility variable.
	final region(:rank==1) rZ = [0:51];
	final dist(:rank==1) dZ = rZ->here;
	final int[:rank==1] Z = new int[dZ];

	for (point(:rank==1) i: [0:7]) { // First 8 subkeys are userkey itself.
	    Z[i] = auserkey[i] & 0xffff;     // Convert "unsigned"
	                                    // short to int.
	}
	...
	return (int value[:rank==1]) new int value[rZ] (point(:rank==1) pt) { return Z[pt]; };
	...
	int j = 0; 
        int k = 0;                 // Index counters.
	int t1 = 0; 
        int t2 = 0; 
        int t3 = 0;           // Temps to hold decrypt subkeys.
	...
	DKR = (int value[:rank==1]) new int value[rDK] (point(:rank==1) i) { return DK[i]; };
     ...
     * fit in 16 bits.
     */
    private void cipher_idea(final byte[:rank==1] text1,
			     final byte[:rank==1] text2,
			     final int value[:rank==1] key) {
	...
	for (point(:rank==1) i: text1.distribution | here) {
	    if (i[0] % 8 == 0) {
		point(:rank==1) i1 = i;      // Index into first text array.
		point(:rank==1) i2 = i;      // Index into second text array.
		...
		x1 = text1[i1] & 0xff;          // Build 16-bit x1 from 2 bytes,
                i1 = i1+[1];
		x1 = x1 | (text1[i1] & 0xff) << 8;  // assuming low-order byte first.
                i1 = i1+[1];
		x2 = text1[i1] & 0xff;
                i1 = i1+[1];
    ...
    private int mul(final int aa, final int ab)  {
        int a = aa;
        int b = ab;
	long p = 0L;             // Large enough to catch 16-bit multiply
