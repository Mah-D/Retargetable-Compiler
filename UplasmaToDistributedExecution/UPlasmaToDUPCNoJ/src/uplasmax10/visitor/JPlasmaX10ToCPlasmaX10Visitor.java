...
public class JPlasmaX10ToCPlasmaX10Visitor implements GJVoidVisitor<String> {
	...
	public void setUseGarbageCollector(boolean useGarbageCollector) {
		this.useGarbageCollector = useGarbageCollector;
	}
	public void setUseDUPDistributed(boolean useDUPDistributed) {
		this.useDUPDistributed = useDUPDistributed;
	}
	public void setUseCalloc(boolean useCalloc) {
		this.useCalloc = useCalloc;
	}
	public void setUseMallocProfiler(boolean useMallocProfiler) {
		this.useMallocProfiler = useMallocProfiler;
	}
	private String getMallocProfiler(String type){
		String temp = "";
		if(useMallocProfiler){
			temp = "(((_task*)_thread_getspecific()) -> total_memory_"+type+") += 4*ceil(0.25 *( ";
		}else{
			return "";
		}
		...
		if (n.type.nodeChoice.choice instanceof NonArrayType
			...
			if (n.expression.nodeChoice.choice instanceof NewObject) {
				if (((symbolTable.lookupField(tempIdent, currentClassName) != null && ((X10Class) (symbolTable
					...
					code.append(" *)  ");
					if(useMallocProfiler){
						code.append("("+getMallocProfiler("value_class")+" sizeof(struct ");
						n.type.accept(this, argu);
						code.append("))),");
					}
					code.append(malloc() + "(sizeof(struct ");
		...
		{   String tempIdent = n.identifier.nodeToken.tokenImage;
			...
			if(!( ( (X10Class) (symbolTable.lookupVariableInScope(tempIdent, blockid, currentClassName, currentMethodName).symbolType)).type == Constants.ClassType.VALUE)  && n.expression.nodeChoice.choice instanceof NewObject ){
				...
				n.type.accept(this, argu);
				 ...
				 code.append(" *) ");
					if(useMallocProfiler){
						code.append("("+getMallocProfiler("class")+" sizeof(struct ");
						n.type.accept(this, argu);
						code.append("))),");
					}
...
/**
	...
	public void visit(PlusExpression n, String argu) {
		...
		if((leftPTN != null && rightPTN !=null) && (leftPTN.type.equals("%s") || rightPTN.type.equals("%s"))){
			...
			if(useMallocProfiler){
				code.append(getMallocProfiler("string")+" 1000 * sizeof(char)))"); code.append(";\n");
			}
			//code.append(" char * "+tempValue +" = (char *)malloc(1000 * sizeof(char));\n");
...
//	}
	...
	public void visit(NewUpdatableArray n, String argu) {
		...
		code.append("))");	
			...
			if(useMallocProfiler){
				code.append(getMallocProfiler("update_array")+"sizeof(int32_t)+("+n.identifier.nodeToken.tokenImage+"*sizeof( ");
						n.nonArrayType.accept(this, argu);
						code.append(valueClass+"))))"+",");
	...
	//	code.append("*)");
		...
		code.append("))");	
			...
			//add size to 
			if(useMallocProfiler){
				code.append(getMallocProfiler("value_array")+"sizeof(int32_t)+("+n.identifier.nodeToken.tokenImage+"*sizeof( ");
						n.nonArrayType.accept(this, argu);
						code.append(valueClass+"))))"+",");
