...
public class TplasmaX10JavaSingleThreaded extends GJDepthFirst<Snippet,Snippet>{
	...
	private String initCodePlaceChecks = "\n\nfor(int i =0; i <Program.MAX_PLACES; ++i){Program.globalPointerHashMap.put(new java.lang.Integer(i), new HashSet());}\n";
	...
	public boolean placeChecksOn = false;
	private enum supportedBackEnds {X10,HJ};
	private supportedBackEnds backEnd = supportedBackEnds.X10;
	...
	public Snippet visit(MainClass n, Snippet argu) {
		...
		tPlasmaCode += generateTabs(1) + programClassName + "." + programClassRunMethod + "("+params+")" + ";\n";
		tPlasmaCode += generateTabs(0) + "}\n";
	...
	public Snippet visit(ProgramClass n, Snippet argu) {
		...
		placeChecksCode +=  "\n static {for(int i =0; i <Program.MAX_PLACES+1; ++i){globalPointerHashMap.put(new java.lang.Integer(i), new HashSet());}}\n";
		}
	...
	public Snippet visit(ConstructorDeclaration n, Snippet argu) {
		...
		String placeChecksCode = "";
		if(placeChecksOn){
			placeChecksCode = " int myUniquePlacePoint";
		}
		_ret.returnTemp = generateTabs(0)+"public "+constructorName+" ( "+tempFormalParamList+placeChecksCode+")\n";//+f5.returnTemp;
	...
	public Snippet visit(MethodDeclaration n, Snippet argu) {
		...
		tempString+=" "+methodName+" "+"( "+tempFormal+placeChecksCode+")";
	...
	public Snippet visit(Block n, Snippet argu) {
		...
		if(inConstructorInit){
			...
			if(backEnd == supportedBackEnds.X10){
				if(!inAsync){

					toAdd+="\n"+ generateTabs(blockDepth)+"((HashSet) Program.globalPointerHashMap.get(new java.lang.Integer(myUniquePlacePoint))).add(new java.lang.Integer("+"this"+".hashCode()));\n";
					}else{

					toAdd+="\n"+ generateTabs(blockDepth)+"((HashSet) Program.globalPointerHashMap.get(new java.lang.Integer("+asyncPoint+".id))).add(new java.lang.Integer("+"this"+".hashCode()));\n";
					}
	...
	/**TODO BE REOMVED THIS NEEDS FIXING PROBABLY A CLASS
	...
	public Snippet visit(FieldAssignment n, Snippet argu) {
		...
		if(!inAsync){
			...
			points=asyncPoint+".id";
		}
		}
		if(placeChecksOn){
		_ret.returnTemp = generateTabs(blockDepth)+generatePlaceCheckCode(points, f0.returnTemp);
		}
	...
	public Snippet visit(Place n, Snippet argu) {
		...
		if(backEnd == supportedBackEnds.X10){
			_ret.returnTemp = "place.places("+_ret.returnTemp+")";
		}else{
			//TODO
		}
	...
	public Snippet visit(DotId n, Snippet argu) {
		...
		if(backEnd == supportedBackEnds.X10){
			_ret.returnTemp = _ret.returnTemp+".id";
		}else{
			//TODO
		}
		return _ret;
	...
	public Snippet visit(DotNext n, Snippet argu) {
		...
		Snippet _ret2 =  new Snippet("", "", new X10Place(), false);
		if(backEnd == supportedBackEnds.X10){
			_ret2.returnTemp = _ret.returnTemp+".next()";
		}else{
			//TODO
		}
	...
	public Snippet visit(DotPrev n, Snippet argu) {
		...
		Snippet _ret2 =  new Snippet("", "", new X10Place(), false);
		if(backEnd == supportedBackEnds.X10){
			_ret2.returnTemp = _ret.returnTemp+".prev()";
		}else{
			//TODO
		}
	...
	public Snippet visit(DotIsFirst n, Snippet argu) {
		...
		Snippet _ret2 =  new Snippet("", "", new X10Boolean(), false);
		if(backEnd == supportedBackEnds.X10){
			_ret2.returnTemp = _ret.returnTemp+".isFirst()";
		}else{
			//TODO
		}
	...
	public Snippet visit(DotIsLast n, Snippet argu) {
		...
		Snippet _ret2 =  new Snippet("", "", new X10Boolean(), false);
		if(backEnd == supportedBackEnds.X10){
			_ret2.returnTemp = _ret.returnTemp+".isLast()";
		}else{
			//TODO
		}
		 ...
		 */n.nodeToken.accept(this, argu);
		 ...
		 if( !classes.containsKey(object) && classSymbolTable.containsKey(object) && !classSymbolTable.get(object).classType.equals(Constants.ClassType.VALUE) ){
			 ...
			 if(!inAsync ){
				 ...
				 tPlasmaCode += generateTabs(blockDepth)+generatePlaceCheckCode(asyncPoint+".id" , object);
			 }}
		 }
	...
	public String generatePlaceCheckCode(String point, String nameOfIdentifier){
		...
		if(!classes.containsKey(nameOfIdentifier)){
			ret =  generateTabs(blockDepth)+"if(!((HashSet)Program.globalPointerHashMap.get(new java.lang.Integer("+point+"))).contains(new java.lang.Integer("+nameOfIdentifier+".hashCode()))){\n"+generateTabs(blockDepth)+"throw new RuntimeException(\"Object in wrong place\");\n"+generateTabs(blockDepth)+"}";
		}
		return ret;
	}
	...
	public Snippet visit(NewObject n, Snippet argu) {
		...
		if(!identifierToAddToGlobalHash.equals("")){
			...
			if(!inAsync){
				...
				toAdd+= generateTabs(blockDepth)+"\n((HashSet) Program.globalPointerHashMap.get(new java.lang.Integer(myUniquePlacePoint))).add(new java.lang.Integer("+identifierToAddToGlobalHash+".hashCode()));\n";
			}else{

				toAdd+= generateTabs(blockDepth)+"\n((HashSet) Program.globalPointerHashMap.get(new java.lang.Integer("+asyncPoint+".id))).add(new java.lang.Integer("+identifierToAddToGlobalHash+".hashCode()));\n";
			}
			//toAdd+="\nelse{throw new RuntimeException(\"Object in wrong place\");\n }";
		}
		}
	...
	/**TODO THIS MAY NEED TO BE FIXED FOR MULTI
	 * nodeToken -> "here"
	 */
	...
	public Snippet visit(PlaceFirstPlace n, Snippet argu) {
		...
		if(backEnd == supportedBackEnds.X10){
			_ret.returnTemp = "place.FIRST_PLACE";
		}else{
			//TODO
		}
	...
	public Snippet visit(PlaceLastPlace n, Snippet argu) {
		Snippet _ret= new Snippet("","Program.MAX_PLACES-1",null,false);
		//_ret = n.nodeToken.accept(this, argu);
		_ret.expType = new X10Integer();
		if(backEnd == supportedBackEnds.X10){
			_ret.returnTemp = "place.LAST_PLACE";
		}else{
			//TODO
		}
		return _ret;
	...
	public Snippet visit(PlaceMaxPlaces n, Snippet argu) {
		...
		_ret.expType = new X10Integer();
		if(backEnd == supportedBackEnds.X10){
			_ret.returnTemp = "place.MAX_PLACES";
		}else{
			//TODO
		}
		return _ret;
